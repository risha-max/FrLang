...
"$PYTHON" main.py 'soit rangée n = rangée [10, 20]; n.ajouter(5); afficher n.taille();' | grep -qx "3"
"$PYTHON" main.py 'definir double(n: nombre) { retourne n * 2; } retourne nombre double(5);' | grep -qx "10"
"$PYTHON" main.py 'definir incrementer(p: pointeur nombre) { valeur(p) = valeur(p) + 1; } soit nombre x = 3; incrementer(adresse(x)); x;' | grep -qx "4"
